...
const CACHE_NAME = 'muamokel-services-v1.0.0';
...
self.addEventListener('fetch', (event) => {
  ...
  // Ignorer TOUTES les requêtes de développement Vite et React
...
async function handleFetchRequest(request) {
  try {
    ...
    if (request.destination === 'image') {
      // Utiliser une image de fallback locale si disponible
      const fallbackImage = await caches.match('/assets/fallback-image.png');
...
async function cacheFirst(request) {
  ...
  try {
    ...
    console.warn('SW: Cache first failed, trying cache fallback:', error);
    // Essayer de récupérer depuis le cache dynamique
    const dynamicCached = await caches.match(request);
    if (dynamicCached) {
      console.log('SW: Dynamic cache fallback hit:', request.url);
  ...
  switch (type) {
    ...
    default:
      if (typeof type === 'undefined') {
        console.warn('SW: Message reçu sans type:', event.data);
      } else {
