...
class AuthService {
  ...
  mockLogin(email, password) {
    ...
    const testUsers = {
      ...
      'test@example.com': {
        email: 'servicebanamokeli@gmail.com',
        password: 'bendelo1996$$$$$',
        name: 'Test User',
        role: 'user',
        id: 3,
  ...
  async loginWithGoogle() {
    ...
    try {
      ...
      // Créer un objet utilisateur compatible avec notre système
      const googleUser = {
        id: user.uid,
        email: user.email,
        name: user.displayName || user.email.split('@')[0],
        role: 'user', // Par défaut user, peut être modifié manuellement
        avatar: user.photoURL,
        provider: 'google'
      };

      // Stocker dans notre système
      this.authToken = await user.getIdToken();
      this.currentUser = googleUser;
      this.isAuthenticated = true;

      roleService.setCurrentUser(googleUser);
      this.storeToken(this.authToken);
      this.startRefreshInterval();

      console.log('✅ Google Login successful for:', googleUser.email);
      return { token: this.authToken, user: googleUser };
    } catch (error) {
  ...
  async registerWithGoogle() {
    // Pour Google, login et register sont identiques
    return this.loginWithGoogle();
  }

  /**
   * Logout de Firebase aussi
   */
  async logout() {
    try {
      // 🧪 Ignorer l'API si en mode mock
      ...
      // Logout Firebase seulement si disponible
      if (isFirebaseAvailable) {
