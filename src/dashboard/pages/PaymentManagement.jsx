...
export default function PaymentManagement() {
  ...
  // CRUD: Ajout ou édition de transaction
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (
      ...
      addNotification('Tous les champs sont obligatoires.', 'error');
      return;
    }
    if (isNaN(form.amount) || Number(form.amount) <= 0) {
      setFormError('Le montant doit être un nombre positif.');
      addNotification('Le montant doit être un nombre positif.', 'error');
      return;
    }
    try {
      if (editingId) {
        // Edition
        paymentManagementService.updateTransaction(editingId, form);
        setFormSuccess('Transaction modifiée avec succès !');
        addNotification('Transaction modifiée avec succès !', 'success');
      } else {
        // Création
        paymentManagementService.addTransaction(form);
        setFormSuccess('Transaction ajoutée avec succès !');
        addNotification('Transaction ajoutée avec succès !', 'success');
      }
      setForm({
        clientName: '',
        clientEmail: '',
  ...
  const handleDeleteTransaction = async (transactionId) => {
    if (
      ...
      if (success) {
        ...
        addNotification('Transaction supprimée.', 'success');
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-6">
      <div className="max-w-7xl mx-auto">
        {/* Formulaire CRUD Paiement */}
        <div className="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20 mb-8">
          <h2 className="text-2xl font-bold text-white mb-4">
            {editingId ? 'Modifier' : 'Ajouter'} une transaction
          </h2>
          <form
          ...
          {formSuccess && (
            <div className="text-green-400 mt-2">{formSuccess}</div>
          )}
        </div>
        {/* Header */}
        <div className="mb-8">
