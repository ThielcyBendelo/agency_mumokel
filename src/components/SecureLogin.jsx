...
const SecureLogin = () => {
  ...
  const {
    ...
    try {
      ...
      if (!res.ok) throw new Error(resp.error || 'Erreur de connexion');
      // Stocker le token et le rôle
      localStorage.setItem('token', resp.token);
      localStorage.setItem('role', resp.user.role);
      localStorage.setItem('user', JSON.stringify(resp.user));
      // Navigation selon le rôle
      if (resp.user.role === 'Administrateur') {
        navigate('/dashboard');
      } else if (resp.user.role === 'Manager') {
        navigate('/users');
      } else {
        navigate('/profile');
      }
      ...
      <div className="w-full max-w-md bg-dark-300/80 backdrop-blur-sm p-8 rounded-2xl shadow-2xl border border-gray-700/50">
        ...
        <div className="mt-6 p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
          <p className="text-blue-300 text-xs leading-relaxed">
            ...
            <br />• Chiffrage SSL/TLS en transmission
          </p>
        </div>
        ...
        <div className="mt-4 p-4 bg-amber-500/10 border border-amber-500/30 rounded-lg">
          <p className="text-amber-300 text-xs leading-relaxed">
            ...
            <em className="text-amber-400">
              Note: À remplacer par votre backend en production
            </em>
          </p>
        </div>
